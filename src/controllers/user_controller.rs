...
pub async fn create_user(database:Data<MongoService>, new_user:Json<CreateUserReq>)->HttpResponse{
    ...
    if user.user_type == UserType::Player{
        ...
        // create player power ups
        let powerups_done =create_player_powerups(&database, &user.email.to_string()).await;
        // creat player stats
        let run_info_done = create_player_stats(database.borrow(), &user.email.to_string() ).await;
        let wallet_done = create_player_wallet(&database, &user.email.to_string()).await;

        if !(run_info_done && powerups_done && wallet_done){
            return HttpResponse::InternalServerError()
                .json(Response{message:"stats and pups not setup".to_string()})
        }
